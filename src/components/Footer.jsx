  ...
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    <div className={`${styles.flexStart} flex-col md:flex-row mb-8 w-full`}>
      <div className='flex flex-1 flex-col justify-start mr-10'>
        ...
        <p className={`${styles.paragraph} mt-4 max-w-[310px]`}>New way to make the payments easy, reliable and secure</p>
      </div>
    ...
    <div className='w-full flex  justify-between items-center pt-6 flex-col md:flex-row border-t-[1px] border-t-[#3f3e45]'>
      <p className='font-poppins font-medium text-[18px] leading-[27px] text-white'>2021 HooBank. All Rights Reserved</p>

      <div className='flex flex-row sm:mt-0 mt-6'>
        {socialMedia.map((social, index) => (
          <img key={social.id} src={social.icon} alt={social.id} className={`w-[21px] h-[21px] object-contain cursor-pointer mx-3`} />
        ))}
      </div>
    </div>
